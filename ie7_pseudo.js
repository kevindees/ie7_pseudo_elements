...
// Otherwise, iterate through all of the keys in the object.
                ...
                for (k in value) {
                    if (Object.prototype.hasOwnProperty.call(value, k)) {
                        v = str(k, value);
                        if (v) {
                            partial.push(quote(toDash(k)) + (gap ? ': ' : ':') + v);
                        }
                    }
                }
            }
...
(function() {
		...
		if($(rule).hasClass(iee)) {
			...
			$(rule).addClass(iee);
			var ieeNew = $(document.createElement(iee+'e')).append( document.createTextNode(content) );
			if(iee == 'ieb') {
				$(rule).prepend(ieeNew); }
			else {
				$(rule).append(ieeNew); }
	...
	for(var i = 0, j = css.length; i < j; i++) {
		...
		for(var x = 0, y = rules.length; x < y; x++) {
			if(rules[x].selectorText.indexOf("ieb") == 0) {
				...
				var newStyles = JSON.stringify(rules[x].style);
				// newStyles.replace(/([A-Z])/g, function($1){return "-"+$1.toLowerCase();})
				// alert( newStyles );
				css[i].addRule(ruleStr + ' iebe', newStyles);
				...
				content = content.substring(0, content.length-1).substring(1);
				var newStyles = JSON.stringify(rules[x].style);
				// camelToDash(newStyles)
				// alert( newStyles );
				css[i].addRule(ruleStr + ' ieae', newStyles);
